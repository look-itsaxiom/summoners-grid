...
export class Game {
    ...
    private setupDemoGame(): void {
        ...
        // Add all card types to main deck (excluding unique generated cards)
        const actionCards = createDemoActionCards().filter(card => 
            !['Follow Me!'].includes(card.name) // Exclude unique action cards generated by effects
        );
        const counterCards = createDemoCounterCards().filter(card => 
            !['Nightmare Pain'].includes(card.name) // Exclude unique counter cards generated by effects
        );
    ...
    public playQuestCard(player: Player, quest: any): boolean {
        if (player.playQuest(quest)) {
            ...
            // Quest goes to active state and waits for manual completion
            // Don't auto-complete immediately
